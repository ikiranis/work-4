...
void insertNodeToArray(node *myNode, int size)
{
    ...
    // Κύληση όλων των στοιχείων μια θέση δεξιά, διαπερνώντας τον πίνακα από το τέλος
...
node * insertNodeToList(node *previous, node *current, int address, int size)
{
    ...
    // Έλεγχος αν έχει δεσμευτεί η μνήμη επιτυχώς
    if (new == NULL)
    {
...
int bestfit(int alloc)
{
    ...
    // Την κρατάμε για να την επιστρέψουμε στο τέλος
    newFreeAddress = mem[memPosition].mem_node->address;
...
void testMemAddRemove()
{
    ...
    int testArray2[10][2] = { {60, 11}, {200, 13}, {1, 4}, {71, 4}, {134, 40}, {259, 37}, {9, 13}, {24, 5}, {10, 2}, {20, 1004} };
